...
The application bundles the following PXE assets. Initial boot files are available locally;
the default menu downloads installation resources from the internet:
...
- The default script is adapted from the `embed.ipxe` menu in the [netboot repository](https://github.com/sky22333/netboot/blob/main/embed.ipxe) and stored at `app/src/main/assets/pxe/autoexec.ipxe`. The app loads it as the initial editable script; TFTP and HTTP serve the session script as both `autoexec.ipxe` and `boot.ipxe`.
...
| `autoexec.ipxe` | `ca3a92e01cd349261a9b8dde8abf423e985d2bece21c8791d8bbedda9aa94c21` |
